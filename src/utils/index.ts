 ...
 * 将对象转成数组
 */
interface ObjectToArrayOptions {
  labelKey?: string | number;
  valueKey?: string | number;
}

export const objectToArray = <T extends object>(obj: T, options: ObjectToArrayOptions = {}) => {
  const { labelKey = "label", valueKey = "value" } = options;
  return Object.keys(obj).map((key) => ({
    [labelKey]: obj[key],
    [valueKey]: key
  })) as Array<{ [K in keyof T]: K extends keyof any[] ? number : T[K] }>;
};
...
/**
 * 将数组转成element [{label: '', value: ''}]形式
 */
export const arrayToObjOpts = <T>(arr: T[], options: ArrayToObjectOptions = {}) => {
  const { labelKey = "label", valueKey = "value" } = options;
